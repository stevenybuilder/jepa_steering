...
from .droid_contract import action_metrics, checkpoint_score, prepare
from .droid_coupling import ARMS, METHOD, Hook, Intervention, arm_fields
...
def native_reference(root, contract):
    ...
    for name, wanted in protocol['source_sha256'].items():
        if sha256(Path(__file__).with_name(name)) != wanted:
...
def checked_inputs(args):
    ...
    if (old['status'] != 'native_droid_full_cem_engineering_passed' or op['contract'] != contract or
            op['source_sha256'] != sha256(Path(__file__).with_name('droid_native.py')) or
