...
from .behavioral_development import ARMS, DEVELOPMENT_SEED, assigned_rows, schedule, verified_report
from .intervention_runner import _model_versions
from .navigation_smoke import CHECKPOINTS, validate_complete
from .planning_contract import prepare
from .planning_native_smoke import run_episode
...
def freeze(vendor, engineering, engineering_code, task, output):
    ...
    for name in ("backends.py", "model_loader.py", "planning_native_smoke.py", "planning_contract.py"):
        if sha256(prior_source / name) != sha256(Path(__file__).with_name(name)):
