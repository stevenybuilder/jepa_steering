...
from .droid_fit_availability import inspect_record,order,CATALOGUE
from .droid_fit_download import download_object
...
def run(args):
    ...
    for name,digest in p['source_sha256'].items():
        if sha256(Path(__file__).with_name(name))!=digest:raise ValueError('Frozen input source changed')
