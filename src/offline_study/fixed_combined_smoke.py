...
from .fixed_combined_check import (COUNTS, HORIZONS, STATUS, deadline, exclusive_json,
    hooks_clean, json_tensors, model_signature, validate_coverage, verify_bindings, verify_files)
...
def verify_numerical(root, expected_hash, checked_source, fit_files, checkpoint, device_uuid):
    ...
    for path in sorted(checked_source.glob("*.py")):
        if path.is_symlink() or path.read_bytes() != (Path(__file__).parent / path.name).read_bytes():
