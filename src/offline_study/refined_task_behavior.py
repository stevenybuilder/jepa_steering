...
from .navigation_coupling_behavior import action_hashes, source_digest
from .navigation_replication import validate_records
from .navigation_smoke import comparison_record
from .operator_fit import _model_versions
from .planning_contract import prepare
from .planning_env_smoke import SingleFitTrajectory
from .planning_native_smoke import SMOKE_SEED, run_episode
from .refined_task_fit import CHECKPOINTS
from .protocol import sha256, write_json
from .pusht_coupling_behavior import paired_inputs as paired_push, validate_push_records
from .pusht_planning_replication import TracedDataset, checked_cohort, input_hashes, trace_segment
...
def task_inputs(args):
    ...
    for name in ('backends.py', 'model_loader.py', 'planning_native_smoke.py', 'planning_contract.py'):
        if sha256(args.reference_source / name) != sha256(Path(__file__).with_name(name)):
