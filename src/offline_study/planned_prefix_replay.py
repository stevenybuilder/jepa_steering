...
from .cem_expansion_pilot import ARMS, TASKS, CHECKPOINT_SHA, INPUT_SHA, model_versions
from .decision_runtime import physics, reset_initial, tensor_hash, jsonable
...
def main():
    ...
    for name,digest in manifest['source_sha256'].items():
        if Path(name).name != name or file_hash(Path(__file__).parent/name) != digest:raise ValueError('Local source changed '+name)
    required_vendor = {
