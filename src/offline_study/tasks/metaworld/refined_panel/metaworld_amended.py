...
Frozen `offline_study.tasks.metaworld.metaworld_component_analysis.load_panel` raises
...
Run against the FROZEN source snapshot (its source hash is bound in the freeze), e.g.
  python src/offline_study/tasks/metaworld/refined_panel/metaworld_amended.py --frozen-src <snapshot>/code/src \
...
def _import_frozen(frozen_src):
    ...
    import offline_study.tasks.metaworld.metaworld_component_analysis as frozen
    from offline_study.evaluation.behavioral_analysis import shard_paths
    from offline_study.evaluation.behavioral_development import assigned_rows, validate_coverage, verified_report
    from offline_study.validation.fixed_response_smoke import verify_episode
    from offline_study.tasks.metaworld.metaworld_component_behavior import ARMS, ENGINEERING, METHOD, TASKS, read, validate_protocol, verify_engineering
