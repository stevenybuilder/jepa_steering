...
from .behavioral_candidate import immutable_source_hash, run_shard
from .behavioral_development import ARMS, DEVELOPMENT_SEED, assigned_rows, verified_report
from .planning_contract import prepare
from .planning_native_smoke import CHECKPOINTS
from .planning_support import PlanningSupportIntervention, TRANSFER_POLICY
...
def launch_contract(args):
    ...
    for name in CRITICAL:
        verify_source_pair(name, sha256(args.engineering_code / "src/offline_study" / name),
                           sha256(Path(__file__).with_name(name)))
