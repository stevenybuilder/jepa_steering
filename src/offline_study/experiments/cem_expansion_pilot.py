...
from offline_study.experiments.steered_cem_pilot import compact_trace, tensor_sha
...
def main():
    ...
    for name,digest in manifest['source_sha256'].items():
        if Path(name).name!=name or file_hash(source_path(name))!=digest:raise ValueError('Source changed '+name)
