...
from offline_study._paths import PACKAGE_ROOT, source_path
...
def validate_manifest(manifest, protocol, source_root, vendor):
    ...
    for name, expected in sources.items():
        if Path(name).name != name or file_hash(source_path(name, source_root)) != expected:
...
def main():
    ...
    validate_manifest(manifest, protocol, PACKAGE_ROOT, args.vendor)
