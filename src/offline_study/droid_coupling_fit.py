...
from .droid_coupling import METHOD,ARMS,Capture,fit_bank,arm_fields
...
def run(args):
    ...
    if (audit['status']!='native_droid_fit_inputs_parity_and_exact_separation_passed' or
            ...
            protocol['source_sha256']!=sha256(Path(__file__).with_name('droid_fit_audit.py'))):
    ...
    try:
        ...
        for name,digest in binding['source_files_sha256'].items():
            if sha256(Path(__file__).with_name(name))!=digest:raise ValueError('Fitting source changed')
