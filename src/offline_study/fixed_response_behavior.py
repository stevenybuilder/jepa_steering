...
from .behavioral_candidate import immutable_source_hash
...
from .fixed_response_smoke import EPISODES, trace_actor, verify_episode, verify_pair
...
def checked_source(root, expected):
    ...
    for path in root.glob("*.py"):
        if sha256(path) != sha256(Path(__file__).with_name(path.name)):
...
def worker_engineering(args):
    ...
    before = device_uuid()
    code = source_hash()
    command = [sys.executable, "-u", "-m", "offline_study.fixed_response_smoke",
    ...
    digest = verify_cem(args.output, args.fits / args.task, Path(__file__).parent, args.task)
