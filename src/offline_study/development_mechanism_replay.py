...
from .candidate_replay_pilot import CaptureFull, ReplayField, components, score_summary
...
from .layer_attention_pilot import PredictorObserver, file_hash
...
def main():
    ...
    for name,sha in manifest['source_sha256'].items():
        if file_hash(Path(__file__).parent/name)!=sha:raise ValueError('Frozen diagnostic source changed:'+name)
