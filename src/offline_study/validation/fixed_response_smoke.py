...
from offline_study.planning.planning_support_smoke import verify_call_schedule
...
def verify_numerical(root, fit, checked_source, task, checkpoint):
    ...
    for path in snapshot_source_files(checked_source):
        ...
        if sha256(path) != sha256(source_path(path.resolve().relative_to(checked_source.resolve()))):
            raise ValueError("Previously checked scientific runtime changed: " + path.name)
        checked_files[path.resolve().relative_to(checked_source.resolve()).as_posix()] = sha256(path)
