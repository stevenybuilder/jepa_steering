...
from .droid_contract import prepare, TRAIN_CONFIG
from .droid_fit_audit import make_raw_dataset, reset
from .droid_fixed_response import ARMS, METHOD, FieldHook, Intervention, make_bank, load_bank
from .droid_native import array_hash, load_model, verified_report
from .protocol import sha256, write_json
from .support_operator import SEED, fit_target, principal_basis, random_basis
...
def input_contract(a):
    ...
    if (audit['status'] != 'native_droid_fit_inputs_parity_and_exact_separation_passed'
            ...
            or audit_protocol['source_sha256'] != sha256(Path(__file__).with_name('droid_fit_audit.py'))
...
def run(a):
    ...
    binding = {'method': METHOD, 'task': 'droid', 'checkpoint_sha256': contract['checkpoint_sha256'],
        **input_binding, 'fit_precision': 'bfloat16', 'block': 6, 'horizon': 3,
        ...
        'source_files_sha256': {name: sha256(Path(__file__).with_name(name)) for name in
