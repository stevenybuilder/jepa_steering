...
from .action_condition_specificity import (file_hash, tensor_hash, dump, initial_population,
    verify_native_sampler, score_vectors, agreement)
...
def main():
    ...
    for name,digest in sources.items():
        if Path(name).name!=name or file_hash(Path(__file__).parent/name)!=digest:raise ValueError('Source bytes changed: '+name)
