...
from .author_runtime import open_normalized_dataset
...
from .navigation_coupling_behavior import ARMS, action_hashes, one_episode, source_digest
from .navigation_replication import validate_records
from .navigation_smoke import comparison_record, validate_complete
from .planning_contract import prepare
from .planning_env_smoke import SingleFitTrajectory
from .planning_native_smoke import CHECKPOINTS, SMOKE_SEED
...
def verify_native_reference(reference, old_source, cohort_hash, planning):
    ...
    for old in old_source.glob("*.py"):
        current = Path(__file__).with_name(old.name)
