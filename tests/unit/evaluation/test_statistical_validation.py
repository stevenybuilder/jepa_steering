...
from offline_study.evaluation.statistical_validation import audit_core, binomial_cdf, exact_binomial_interval, holm, paired_exact_p, paired_sample_size
...
class StatisticalValidationTests(unittest.TestCase):
    ...
    def test_frozen_core_counts_and_no_mutation(self):
        path = Path(__file__).resolve().parents[3] / 'artifacts/offline_study/core-completion-preservation-20260908-v1/ANALYSIS_REPORT.json'
        if not path.exists():
