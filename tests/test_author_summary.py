...
from offline_study.author_summary import verified_analysis
from offline_study.author_merge_extension import diagnostic_coverage
...
class SummaryTests(unittest.TestCase):
    ...
    def test_mechanism_merge_rejects_omissions_and_duplicates(self):
        expected = [{"trajectory_id": "a", "start": 0}]
        row = {**expected[0], "metrics": {"visual_output_interaction_mse_h6": 0.}}
        diagnostic_coverage([row], expected, "vision_action_coupling")
        for rows in ([], [row, row], [{**expected[0], "metrics": {}}]):
            with self.assertRaises(ValueError):
                diagnostic_coverage(rows, expected, "vision_action_coupling")
