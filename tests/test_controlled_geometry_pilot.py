...
from offline_study.controlled_geometry_pilot import (
    ANCHORS, OFFSETS, CaptureH3, action_batch, predictor_precision,
    summarize_fields, tensor_hash, write_json, validate_manifest,
    REQUIRED_SOURCE, file_hash, tree_hash,
)
...
def test_exclusive_receipts_and_protocol():
    protocol = json.loads((Path(__file__).parents[1] / 'paper/data/controlled_geometry_protocol.json').read_text())
