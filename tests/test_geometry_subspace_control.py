...
from causal_planner_forks import counterfactual_actions, execute_fork, load_development_bank, replay_action_count, restore_cached_goal, visual_pooled
...
def test_beta_zero_exact_identity_and_hook_removal():
    ...
    torch.testing.assert_close(patch.first_activation["before_pooled"], patch.first_activation["after_pooled"], atol=0, rtol=0)
    assert patch.first_activation["realized_raw_l2"].eq(0).all()
...
def test_first_step_only_resets_each_rollout_and_preserves_token_residuals():
    ...
    assert patch.calls == 2
    torch.testing.assert_close(patch.first_activation["before_pooled"], value.mean(1), atol=0, rtol=0)
    torch.testing.assert_close(patch.first_activation["after_pooled"], first[0].mean(1), atol=0, rtol=0)
    assert patch.first_activation["requested_standardized_orthogonal_l2"].max() < 2e-6
    assert patch.first_activation["realized_standardized_orthogonal_l2"].max() < 2e-6
...
def test_cached_goal_restoration_preserves_original_values_without_aliasing():
    generated = {"visual": torch.ones(1, 3, 2, 2).float(), "proprio": torch.ones(1, 4)}
    cached = {"visual": torch.full((1, 3, 2, 2), 7, dtype=torch.uint8), "proprio": torch.full((1, 4), .25)}
    restored = restore_cached_goal(generated, cached)
    torch.testing.assert_close(restored["visual"], cached["visual"].float(), atol=0, rtol=0)
    torch.testing.assert_close(restored["proprio"], cached["proprio"], atol=0, rtol=0)
    restored["visual"].zero_()
    restored["proprio"].zero_()
    assert cached["visual"].eq(7).all() and cached["proprio"].eq(.25).all()
    assert generated["visual"].eq(1).all()
