#!/bin/bash
# run_v09_eval.sh — protocol v0.9 randomised-factor factorial (cross model design jepa.md, A-2 behaviour gate).
#   Waits for V09_FACT_DONE (run_v08_factorial.sh), merges every batch per arm (merge_heldout.py, hash split, label
...
#   Discovery scenes only; confirmation seeds of the v0.8 split stay sealed. Marker: V09_EVAL_DONE in logs/drive_v09_eval.log.
set -u
...
FACT=$BASE/artifacts/drive_factorial_v09; MERGED=$BASE/artifacts/drive_v09_merged; EVAL=$BASE/artifacts/drive_v09_eval
MODELS=$BASE/artifacts/drive_models; FEAT=$BASE/artifacts/drive_features
LOGDIR=$BASE/logs; LOG=$LOGDIR/drive_v09_eval.log; FLOG=$LOGDIR/drive_factorial_v09.log
SEEDS=${DRIVE_SEEDS:-"0 1 2"}; STATIC=${V09_STATIC:-"rand"}; DYN=${V09_DYN:-""}
KEEP_CACHE_SEEDS=${V09_KEEP_CACHE_SEEDS:-"0"}   # latent caches kept for these model seeds (relational transport); others deleted (disk)
...
mkdir -p $MERGED $EVAL
if [ "${V09_SKIP_WAIT:-0}" != "1" ]; then say "waiting for V09_FACT_DONE in $FLOG"; until grep -q V09_FACT_DONE $FLOG 2>/dev/null; do sleep 60; done; fi
...
for b in $STATIC $DYN; do for ARM in A B; do
  ...
  $DPY $CODE/merge_heldout.py --root $FACT/$b/arm$ARM --output $MO --protocol cgs-metadrive-pilot-v0.9 --domain driving \
    --label-validation $LAB --split-seed 0 --exclude-seeds >> $LOGDIR/drive_v09_merge.log 2>&1 || { say "merge $b arm$ARM FAILED (see drive_v09_merge.log)"; rm -rf $MO; continue; }
...
eval_model() { # eval_model SETNAME ARM SEED
  local SET=$1 ARM=$2 s=$3; local MD=$MODELS/arm${ARM}_seed$s SRC=$MERGED/$SET/arm$ARM EV=$EVAL/$SET/arm${ARM}_seed$s WLOG=$LOGDIR/drive_v09_wave_${SET}_arm${ARM}_seed$s.log
...
EOF
  DRIVE_SKIP_WAIT=1 DRIVE_SKIP_MERGE=1 DRIVE_ROOT=$FACT/rand/arm$ARM DRIVE_OUT=$EV DRIVE_CODE=$CODE DRIVE_REPO=$REPO DRIVE_MPY=$MPY DRIVE_DPY=$DPY \
    ...
    DRIVE_TRAIN_REF=$FEAT/arm$ARM/training_reference.npz DRIVE_PRIMARY_LEVEL=$LEVEL DRIVE_LOG=$WLOG bash $CODE/run_wave_drive.sh $ARM v09_$SET
