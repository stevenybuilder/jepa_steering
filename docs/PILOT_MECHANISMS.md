...
Status: all 64 frozen development cases passed compact-source, coverage, and parity validation on 2026-09-13: IDs 0–31 in each MetaWorld task, n=32 per task. The separate actual-CEM addon also passed with all eight fixed cases, n=4 per task. No partial-cohort means are reported. At this aggregate snapshot, seven initial cases have explicitly verified compact-only archives while their bulk H6 preservation remains pending; [the source receipt](../paper/data/pilot_summary.json) is authoritative for preservation status.
