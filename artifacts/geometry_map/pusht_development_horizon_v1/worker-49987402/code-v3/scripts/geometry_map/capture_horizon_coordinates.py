...
def capture_episode(args, row, wm, preprocessor, cfg, readout):
    ...
    try:
        ...
        old_error = float((prefix.cpu().double()-old.double()).abs().max())
        ...
        close_env(env)
    env, repeat_agent, repeat_z, repeat_checks = reconstruct_fresh(cfg, wm, preprocessor, bank)
    try:
    ...
    value = {"complete": True, "episode": bank["episode"], "split": "development", "replan": 0,
             ...
             "raw_context_visual": bank["replans"][0]["observation_visual"],
             "raw_context_proprio": bank["replans"][0]["observation_proprio"], "goal": bank["goal"],
