...
files=[dict(path=str(p.relative_to(r)),bytes=p.stat().st_size,sha256=sha(p))for p in sorted(r.rglob('*'))if p.is_file()and '__pycache__'not p.parts];print(json.dumps(dict(files=files,checkpoint_sha256=sha(checkpoint))))
""".replace('ROOT',repr(r))
