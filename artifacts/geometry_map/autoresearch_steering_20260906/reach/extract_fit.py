    ...
    for directory in args.sources:
        ...
        for p in sorted(root.glob('episode-*-unsteered-replan-*.pt')):
            ...
            for truth in t['actual_executed_prefix_encodings']:
                ...
                y=torch.cat([pool(truth['visual']-pred['visual'],384),pool(truth['proprio']-pred['proprio'],16)])
...
def fit(args):
    ...
    ridge=10*len(Z);W=Z.T@np.linalg.solve(Z@Z.T+ridge*np.eye(len(Z)),Y)
    gen=np.random.default_rng(20260906);perm=np.r_[gen.permutation(384),384+gen.permutation(16)];sign=gen.choice([-1.,1.],400)
    pred=Z@W
    out=Path(args.output);out.mkdir(parents=True,exist_ok=False);np.savez(out/'correction.npz',mean=mean,scale=scale,coef=W.astype(np.float32),perm=perm,sign=sign.astype(np.float32))
    (out/'DONE.json').write_text(json.dumps(dict(complete=True,episodes=sorted(set(units[:,0].tolist())),rows=len(X),ridge=ridge,inputs=[{'path':p,'sha256':sha(p)}for p in args.sources],train_native_pooled_mse=float(np.mean(Y**2)),train_corrected_pooled_mse=float(np.mean((Y-.5*pred)**2)),parameters=sha(out/'correction.npz')),indent=2));print((out/'DONE.json').read_text())
