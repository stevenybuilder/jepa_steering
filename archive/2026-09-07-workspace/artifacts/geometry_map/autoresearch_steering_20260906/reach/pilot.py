...
class Correction:
    ...
    def vector(self,z,pred,actions,h):
        ...
        zf=(f-self.d['mean'])/self.d['scale'];return torch.cat([torch.ones((n,1),device=f.device),zf],1)@self.d['coef']
    def sham(self,delta):return delta[:,self.d['perm']]*self.d['sign']
    def delta_cost(self,pred,goal,delta,h,alpha):
        n=len(delta);v=pred['visual'][h].reshape(n,-1,384).mean(1)-pool(goal['visual'],384)
        p=pred['proprio'][h].reshape(n,-1,16).mean(1)-pool(goal['proprio'],16)
        return (2*v*delta[:,:384]+delta[:,:384]**2).mean(1)+alpha*(2*p*delta[:,384:]+delta[:,384:]**2).mean(1)
    ...
    try:
        ...
        for e in a.episodes:
            ...
            for lo in range(0,301,16):
                ...
                assert torch.allclose(delta[:,:384].norm(dim=1),sham[:,:384].norm(dim=1),atol=1e-6,rtol=1e-6)
                assert torch.allclose(delta[:,384:].norm(dim=1),sham[:,384:].norm(dim=1),atol=1e-6,rtol=1e-6)
                ...
                if lo==0:
                    altered=pred.clone()
                    for k,d in [('visual',delta[:,:384]),('proprio',delta[:,384:])]:altered[k][h]+=d.reshape(d.shape[0],*[1]*(altered[k][h].ndim-2),d.shape[-1])
                ...
                if lo<=300<lo+len(aa[0]):
                    ...
                    for truth in t['actual_executed_prefix_encodings']:
                        ...
                        for k,sl,dim in [('visual',slice(0,384),384),('proprio',slice(384,400),16)]:
                            ...
                            dd=d[sl].reshape(*[1]*(pp.ndim-1),dim);sd=ss[sl].reshape_as(dd)
