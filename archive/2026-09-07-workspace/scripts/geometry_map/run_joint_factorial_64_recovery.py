...
def run_source(args,row,wm,prep):
    ...
    write(args.output/'MODEL_COMPLETE.json',dict(complete=True,episode=row['key'],forward_stage_seconds=time.monotonic()-start,all_forward_guards_passed=True,all11arms_saved_to_cpu_memory=True,scientific_outputs_not_yet_durable=True))
    print(json.dumps(dict(event='model_stage_complete',episode=row['key'],seconds=time.monotonic()-start)),flush=True)
