...
def main():
    global DRIVE
    parser=argparse.ArgumentParser();parser.add_argument('--extended',action='store_true')
    parser.add_argument('--execution',action='store_true',help='Include verified parallel execution and priority handoff preparations')
    args=parser.parse_args();extended=args.extended or args.execution
    ...
    if args.execution:
        DRIVE='gdrive:Research-Archives/JEPA-WM/live-20260908T122000Z/preparation'
        parents += ['droid-coupling-preparation-20260908-v3',
            'droid-parallel-worker-20260908-v3-retry-v2', 'droid-relay-20260908-v2',
            'pointmaze-history-relay-20260908-v1', 'pointmaze-tx-resume-20260908-v1',
            'pointmaze-seed236-tx-20260908-v1',
            *['routing-priority-preparation-20260908-v'+str(i) for i in range(1,4)]]
    files=set()
    for name in parents:
    ...
    if args.execution:
        files.update(ROOT/p for p in ('reports/ABLATION_COVERAGE_STATUS.md','docs/POINTMAZE_TRAINING_HISTORY.md'))
    ...
    output=ROOT/('artifacts/offline_study/preparation-drive-backup-20260908-v3' if args.execution else
                 'artifacts/offline_study/preparation-drive-backup-20260908-v2' if extended else
    ...
    subprocess.run(['rclone','copyto',str(archive),destination,'--immutable','--drive-chunk-size','8M',
        '--tpslimit','2','--retries','5','--low-level-retries','10'],check=True)
    verify_archive(['rclone','cat',destination],digest,size,manifest)
    if any(sha(ROOT/name)!=row['sha256'] or (ROOT/name).stat().st_size!=row['bytes'] for name,row in manifest.items()):
        raise ValueError('Selected preparation/source member changed during snapshot; preserve unverified attempt')
