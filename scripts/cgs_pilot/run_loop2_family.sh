#!/bin/bash
# Loop 2 family pipeline (protocol v0.4): <group> <control_min_m>. CPU stages run immediately; GPU stages wait for wave-2 mech to finish.
set -u
G=$1; C=$2
ROOT=/root/cgs-pilot/artifacts/family_$G
OUT=/root/cgs-pilot/artifacts/family_${G}_merged
MECH=/root/cgs-pilot/artifacts/mech_$G
CODE=/root/cgs-pilot/code/cgs_pilot
RPY=/opt/conda/envs/robocasa/bin/python; MPY=/opt/conda/bin/python
...
mkdir -p $OUT $MECH $ROOT/_validation
echo "[$(date -u +%FT%TZ)] loop2 $G start (control $C)" > $LOG
cd $CODE
# --- CPU stages ---
$RPY augment_null_factor.py --root $ROOT --only-passing --obj-group $G --control-min-m $C > /root/cgs-pilot/logs/augment_$G.log 2>&1
echo "[$(date -u +%FT%TZ)] augment done: $(grep -c augmented /root/cgs-pilot/logs/augment_$G.log)" >> $LOG
$RPY validate_hazard_labels.py --root $ROOT --obj-group $G --control-min-m $C --output $ROOT/_validation/hazard_label_validation.json > /root/cgs-pilot/logs/validate_$G.log 2>&1
echo "[$(date -u +%FT%TZ)] validate done: $(grep -c '"passed": true' /root/cgs-pilot/logs/validate_$G.log) pass / $(grep -c '^{' /root/cgs-pilot/logs/validate_$G.log)" >> $LOG
for d in $ROOT/seed_*/; do s=$(basename $d | cut -d_ -f2); [ -f $d/manifest.jsonl ] || continue
  if grep -q '"replay_frames_bit_exact": false' $d/manifest.jsonl && [ ! -f $ROOT/_validation/render_jitter_seed_$s.json ]; then
    $RPY -c "import robocasa_contact_pilot as g; g.OBJ_GROUP='$G'; g.CONTROL_MIN_M=$C; import runpy,sys; sys.argv=['verify_render_jitter.py','--stimulus-dir','$d','--output','$ROOT/_validation/render_jitter_seed_$s.json']; runpy.run_path('verify_render_jitter.py', run_name='__main__')" >> /root/cgs-pilot/logs/render_jitter_$G.log 2>&1; fi; done
...
echo "[$(date -u +%FT%TZ)] merged: $(grep -c . $OUT/manifest.jsonl) cells; discovery $(tr '\n' ' ' < $OUT/discovery_seeds.txt)" >> $LOG
echo CPU_DONE >> $LOG
# --- GPU stages (after wave-2 mechanism stage) ---
until grep -q MECH_DONE /root/cgs-pilot/logs/mech_w2.log 2>/dev/null; do sleep 600; done
